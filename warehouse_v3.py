...
# ---------- MQTT LED command publishing from Jetson ----------
# Jetson is the edge controller for physical LED indicators.
# It publishes simple commands to the MQTT broker; ESP32/Arduino subscribes and drives LEDs.
WAREHOUSE_MQTT_LED_ENABLED = os.environ.get("WAREHOUSE_MQTT_LED_ENABLED", "false").strip().lower() in ("1", "true", "yes", "on")
...
def init_mqtt_led_client():
    """
    Start MQTT client for LED commands. Failure must not stop the camera/AI flow.
    """
    global _mqtt_led_client, _mqtt_led_ready

    if not WAREHOUSE_MQTT_LED_ENABLED:
        print("[MQTT-LED] Disabled. Set WAREHOUSE_MQTT_LED_ENABLED=true to enable Jetson -> MQTT -> ESP32.")
        return

    if not WAREHOUSE_MQTT_HOST:
        print("[MQTT-LED] Disabled: WAREHOUSE_MQTT_HOST is empty")
        return

    try:
        import paho.mqtt.client as mqtt

        client_id = os.environ.get("WAREHOUSE_MQTT_CLIENT_ID", f"jetson-warehouse-led-{uuid.uuid4().hex[:8]}")
        ...
        def on_connect(client, userdata, flags, rc):
            ...
            print(f"[MQTT-LED] Connected rc={rc} host={WAREHOUSE_MQTT_HOST}:{WAREHOUSE_MQTT_PORT} tls={WAREHOUSE_MQTT_TLS}")

        def on_disconnect(client, userdata, rc):
            global _mqtt_led_ready
            _mqtt_led_ready = False
            print(f"[MQTT-LED] Disconnected rc={rc}")
        ...
        print(f"[MQTT-LED] Started client_id={client_id}, topic_prefix={WAREHOUSE_MQTT_TOPIC_PREFIX}")

    except Exception as e:
        _mqtt_led_client = None
        _mqtt_led_ready = False
        print(f"[MQTT-LED] Init failed: {e}")
...
# ---------- End MQTT LED command publishing from Jetson ----------
